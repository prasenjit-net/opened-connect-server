...
function AdminOverview() {
 ...
 return <div className="flex flex-col gap-5">
  ...
  {data && <>
   ...
   <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{activityKinds.map(kind => <Link key={kind} to={`/activity/${kind}`} className="card transition-colors hover:bg-surface-2"><h3 className="text-sm font-medium text-ink-muted">{activityLabels[kind]}</h3><p className="my-3 text-3xl font-semibold">{data.counts[kind].active}<span className="ml-2 text-sm font-normal text-ink-muted">active</span></p><p className="text-xs text-ink-faint">{data.counts[kind].total} retained · {data.counts[kind].revoked} revoked · {data.counts[kind].expired} expired</p></Link>)}</div>
   <section className="card"><div className="card-head"><h2>Recent retained activity</h2></div><p className="mb-4 text-sm text-ink-muted">The 10 newest records across transactions, codes, tokens, and consents. Expired records may be removed during cleanup.</p><ActivityTable records={data.recent} /></section>
  </>}
 </div>;
}
