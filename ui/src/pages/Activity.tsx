...
export default function ActivityPage({ kind }: { kind: ActivityKind }) {
    ...
    const revoke = useMutation({
        mutationFn: (record: ActivityRecord) => api.revokeActivity(kind, record.id), onError: error => {
    ...
    return <div className="flex flex-col gap-4">
        ...
        <section className="card">
            <form className="mb-4 flex flex-wrap items-end gap-3" onSubmit={e => { e.preventDefault(); setFilters({ ...draft, page: 1 }); }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={query.isFetching}>Search</button>
            ...
            {result && <><ActivityTable records={result.records} busy={query.isFetching || revoke.isPending} onRevoke={record => { revoke.reset(); setSelected(record); }} />
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm text-ink-muted"><span>{result.total} records · Page {result.page} of {pages} · 10 per page</span><div className="flex gap-2"><button type="button" className="btn btn-secondary btn-sm" disabled={query.isFetching || result.page <= 1} onClick={() => setFilters({ ...filters, page: result.page - 1 })}>Previous</button><button type="button" className="btn btn-secondary btn-sm" disabled={query.isFetching || result.page >= pages} onClick={() => setFilters({ ...filters, page: result.page + 1 })}>Next</button></div></div>
            </>}
        </section>
        {selected && <section ref={confirmation} tabIndex={-1} role="dialog" aria-modal="false" aria-labelledby="revoke-title" className="card border border-err">
            ...
            <div className="flex gap-2"><button type="button" className="btn btn-danger" disabled={revoke.isPending || (revoke.error instanceof ApiError && revoke.error.code === "ACTIVITY_NOT_ACTIVE")} onClick={() => revoke.mutate(selected)}>{revoke.isPending ? "Revoking…" : "Confirm revoke"}</button><button type="button" className="btn btn-secondary" disabled={revoke.isPending} onClick={() => setSelected(null)}>Cancel</button></div>
        </section>}
    </div>;
}
