...
export default function ConsentScreen({ clientName, scopes, busy, error, onApprove, onDeny }: {
  ...
  return <section className="card w-full min-w-0 max-w-[420px] p-5 sm:p-8">
    <h1 className="min-w-0 break-words text-xl font-semibold [overflow-wrap:anywhere]">{clientName}</h1>
    ...
    <ul className="mb-6 flex flex-col gap-2 text-sm">
      {scopes.map((s) => <li key={s.scope} className="flex items-start gap-2">
        ...
        <span className="min-w-0 [overflow-wrap:anywhere]">{s.description || s.scope}</span>
      </li>)}
    </ul>
    {error && <p role="alert" className="mb-4 break-words rounded-lg bg-err-soft p-3 text-sm text-err">{error}</p>}
