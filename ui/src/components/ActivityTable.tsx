 ...
 return <div className="min-w-0 max-w-full overflow-x-auto rounded-lg border border-line" aria-busy={busy}>
  <table className="w-full min-w-[880px] table-fixed text-left text-sm">
   ...
   <tbody>{records.map(record => <tr key={record.id} className="border-t border-line align-top hover:bg-surface-2">
    <td className="px-4 py-3"><span className="font-medium">{activityLabels[record.kind]}</span><details className="mt-1 text-xs text-ink-muted"><summary className="cursor-pointer">Record details</summary><p className="mt-2 max-w-52 break-all font-mono">{record.id}</p>{record.idTokenExpiresAt && <p className="mt-2">ID token expires: {activityDate(record.idTokenExpiresAt)}</p>}{record.grantType && <p className="mt-2 break-all">Grant: {record.grantType}</p>}{record.audience && <p className="mt-2 break-all">Audience: {record.audience}</p>}{record.familyId && <p className="mt-2 break-all">Family: {record.familyId}</p>}{record.absoluteExpiry && <p className="mt-2">Absolute expiry: {activityDate(record.absoluteExpiry)}</p>}{record.idleExpiry && <p className="mt-2">Idle expiry: {activityDate(record.idleExpiry)}</p>}</details></td>
    <td className="px-4 py-3"><Link to="/clients/$clientId" params={{ clientId: record.clientId }} className="block break-words text-accent [overflow-wrap:anywhere] hover:underline">{record.clientName || record.clientId}</Link><div className="mt-1 text-xs text-ink-muted">{record.userId ? <Link to="/users/$userId" params={{ userId: record.userId }} className="break-words hover:underline [overflow-wrap:anywhere]">{record.userEmail || record.userName || record.userId}</Link> : record.subjectKind === "client" ? "Machine client" : "Awaiting sign-in"}</div></td>
